-- ============================================================
-- RESTORE FULL BACKUP — Atomic restore in single transaction
-- SECURITY: auth.uid() ile tenant dogrulama
-- HARDENED: full counters, skip logic, NULL validation
-- ============================================================
...
BEGIN
    ...
    IF jsonb_typeof(backup->'products') = 'array' THEN
        ...
        LOOP
            ...
            -- Duplicate check: name + tenant_id
    ...
    IF jsonb_typeof(backup->'sales') = 'array' THEN
        ...
        LOOP
            -- NULL validation: date ve total zorunlu
            ...
            -- Duplicate check: tenant + date + total + cash + card + notes
            ...
            WHERE tenant_id = v_tenant_id
              ...
              AND COALESCE(notes, '') = COALESCE(v_sale->>'notes', 'Backup restore')
            ...
            ELSE
                INSERT INTO sales (
                    ...
                    COALESCE(v_sale->>'notes', 'Backup restore'),
                    false
                )
    ...
    IF jsonb_typeof(backup->'product_sales') = 'array' THEN
        ...
        LOOP
            -- NULL validation: sale_id ve product_id zorunlu
            ...
            -- Duplicate check: sale_id + product_id
            IF EXISTS (
            ...
            -- Get date from sales (single source of truth)
