-- ============================================================
-- 025 — RESTORE FULL BACKUP (notes-duplicate bug fix)
-- 012_restore_full_backup.sql'i ESKITIR. Bu dosya CREATE OR REPLACE
-- ile ayni fonksiyonu yeniden yazar.
--
-- BUG: Eski versiyonda sales duplicate check ve INSERT'te
--      COALESCE(notes, 'Backup restore') kullaniliyordu.
--      Sonuc: backup'taki notes=NULL kayitlar ilk restore'da
--      'Backup restore' yazilarak girdi. Ikinci restore'da
--      eski karsilastirma artik tutmadi -> DUPLICATE SATIS.
--
-- FIX: notes karsilastirmasi ve INSERT'te orijinal NULL korunur.
--      COALESCE(notes, '') ile bos string normalize edilir
--      (sadece karsilastirmada).
-- ============================================================
...
BEGIN
        ...
        LOOP
            ...
            ELSE
                INSERT INTO products (
                    tenant_id, name, price, cost, is_active, is_deleted, category_id, category_name
                ) VALUES (
                    v_tenant_id,
                    v_product->>'name',
                    ...
                    COALESCE(
                        ...
                        (SELECT name FROM categories WHERE id = NULLIF(v_product->>'category_id','')::uuid LIMIT 1)
                    )
                )
    ...
    -- ========================================================
    -- 2) SALES — notes NULL safety (BUG FIX)
    -- ========================================================
    IF jsonb_typeof(backup->'sales') = 'array' THEN
        ...
        LOOP
            ...
            v_backup_notes := v_sale->>'notes';   -- NULL ise NULL kalir

            -- Duplicate check: notes karsilastirmasi NULL-safe
            -- COALESCE(notes,'') = COALESCE(v_backup_notes,'')
            -- INSERT'te de orijinal NULL korunacak (asagida).
            ...
            ELSE
                INSERT INTO sales (
                    ...
                    v_backup_notes,   -- NULL ise NULL gir, fallback YOK
                    false
                )
        ...
        LOOP
            ...
            INSERT INTO expenses (
                ...
                COALESCE(
                    ...
                    (SELECT name FROM categories WHERE id = NULLIF(v_expense->>'category_id','')::uuid LIMIT 1)
                )
            );
...
-- TEST
-- ============================================================
-- 1) Backup'ta notes=null olan satis al, restore et.
--    Sales tablosunda notes=NULL gormelisin (eski versiyonda
--    'Backup restore' yaziyordu).
-- 2) Ayni backup'i ikinci kez restore et.
--    skipped.sales = inserted.sales (1. tur), inserted.sales=0
--    olmali. Eski versiyonda duplicate olusurdu.
-- ============================================================
