-- ============================================================
-- 029 — RESTORE FULL BACKUP (sales idempotency_key fix)
-- 028 ESKITILDI.
--
-- DEGISIKLIK: SADECE sales restore blogu yeniden yazildi.
--   - idempotency_key deterministik md5 ile hesaplaniyor
--   - INSERT ... ON CONFLICT (idempotency_key) DO NOTHING
--   - RETURNING NULL ise key ile mevcut kaydi bul
--   - EXCEPTION block kaldirildi (artik gereksiz)
--
-- DIGER BLOKLAR (products, product_sales, expenses) 028 ile
-- AYNI — degistirilmedi.
-- ============================================================
...
BEGIN
    ...
    -- ========================================================
    -- 1) PRODUCTS — duplicate-safe (DEGISMEDI)
    -- ========================================================
    ...
    -- ========================================================
    -- 2) SALES — IDEMPOTENCY KEY + ON CONFLICT (YENIDEN YAZILDI)
    -- ========================================================
    IF jsonb_typeof(backup->'sales') = 'array' THEN
        ...
        LOOP
            ...
            -- Ayni tenant + date + total + notes -> ayni key
            v_idem_key := md5(
                v_tenant_id::text                                          || '|' ||
                v_sale_date::text                                          || '|' ||
            ...
            -- RETURNING NULL ise duplicate -> mevcut kaydi bul
            ...
            -- Mapping kur (eski sale_id -> yeni sale_id)
    ...
    -- ========================================================
    -- 3) PRODUCT_SALES — duplicate-safe (DEGISMEDI)
    -- ========================================================
    ...
    -- ========================================================
    -- 4) EXPENSES — duplicate-safe (DEGISMEDI)
    -- ========================================================
...
-- TEST
-- ============================================================
-- 1) Bos tenant'a restore:
--    inserted.sales = N, skipped.sales = 0, exception YOK
--
-- 2) Ayni backup'i ikinci kez restore:
--    inserted.sales = 0, skipped.sales = N, exception YOK
--    UI: "0 satis | N atlandi"
--
-- 3) sales_idempotency_unique constraint devrede iken:
--    ON CONFLICT yakaliyor, hata YOK.
--
-- 4) v_sale_map dogru kuruldu mu?
--    -> product_sales restore edilirken bagli sale'lar bulunmali
-- ============================================================
